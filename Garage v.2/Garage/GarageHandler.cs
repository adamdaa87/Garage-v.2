...
namespace Garage_v._2.Garage
{
    public class GarageHandler
    {
        ...
        private Menu GetMenuTree()
        {
            ...
            var mainMenu = Menu.Create("Garage ver 0.2    Main menu");
        ...
        private void ParametricSearch()
        {
            ...
            foreach (var v in garage)
            {
                ...
                if (regNo != "" && v.RegNo.ToUpper() != regNo.ToUpper())
                    match = false; // miss
                if (color != "" && v.Color.ToUpper() != color.ToUpper())
                    match = false; // miss
                if (nrOfWheels != -1 && v.NrOfWheels != nrOfWheels)
                    match = false; // miss
                if (fuelType != "" && v.FuelType.ToUpper() != fuelType.ToUpper())
                    match = false; // miss
                if (match)
                {
        ...
        private void ParkSomeVehicles()
        {
            ...
            ParkVehicle(new Car("HUB981", "Red", 4, "Gasoline", "BMW"));
